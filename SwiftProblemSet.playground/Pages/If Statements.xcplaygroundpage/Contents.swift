...
func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    if bugs < 10 && hasMusic && numberOfLevels > 5 {
        ...
        if bugs > 9 {
            print("\(game) has too many bugs for release and was rejected")
        }
        
        if !hasMusic {
            print("\(game) has no music and was rejected")
        }
        
        if numberOfLevels <= 5 {
            print("\(game) has too few levels for release and was rejected")
        }
    }
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    if canFinishRun && canFinishBike {
        print("Congratulations \(name)! You're ready for the duathlon.")
    } else {
        
        if !canFinishBike && canFinishRun {
            print("When you can finish the biking section \(name), you'll be ready for the duathlon.")
        }
        
        if !canFinishRun && canFinishBike {
            print("When you can finish the running section \(name), you'll be ready for the duathlon.")
        }
        
        if !canFinishRun && !canFinishBike {
            print("When you can finish the running and biking section \(name), you'll be ready for the duathlon.")
        }
    }
}
